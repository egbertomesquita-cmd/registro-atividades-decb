...
const PUBLIC_URL='https://egbertomesquita-cmd.github.io/registro-atividades-decb/';
let session=null,profile=null,items=[];
...
$('#google-login').onclick=async()=>{const btn=$('#google-login');btn.disabled=true;try{const {error}=await db.auth.signInWithOAuth({provider:'google',options:{redirectTo:PUBLIC_URL}});if(error)throw error}catch(err){toast(err.message||'Não foi possível iniciar o acesso pelo Google.',true);btn.disabled=false}};
...
$('#open-citation').onclick=()=>$('#citation-dialog').showModal();
$('#copy-citation').onclick=async()=>{try{await navigator.clipboard.writeText($('#citation-text').textContent.trim());toast('Referência copiada.')}catch{toast('Não foi possível copiar automaticamente.',true)}};
